...
enum { TITLE_COLUMN, ICON_COLUMN, NAME_COLUMN, EDITOR_COLUMN, N_COLUMNS };
...
gboolean get_current_iter(MarkerWindow *window, GtkTreeIter *iter) {
  GtkTreeModel *model = GTK_TREE_MODEL(window->documents_tree_store);
  GtkTreeSelection *selection =
      gtk_tree_view_get_selection(window->documents_tree_view);
 ...
 *
 * Returns: TRUE if the user would like to proceed without saving. FALSE if the
 * user wants to cancel the operation.
 */
static gboolean show_unsaved_documents_warning(MarkerWindow *window) {
  ...
  adw_message_dialog_add_responses(ADW_MESSAGE_DIALOG(dialog), cancel_text,
                                   GTK_RESPONSE_CANCEL, ok_text,
                                   GTK_RESPONSE_OK, (char *)0);
...
static void action_fullscreen(GSimpleAction *action, GVariant *value,
                              gpointer window) {
...
static void action_sidebar(GSimpleAction *action, GVariant *value,
                           gpointer window) {
  gboolean state = g_variant_get_boolean(value);
...
static void action_link(GSimpleAction *action, GVariant *parameter,
                        gpointer window) {
...
static void action_monospace(GSimpleAction *action, GVariant *parameter,
                             gpointer window) {
  MarkerEditor *editor = marker_window_get_active_editor(MARKER_WINDOW(window));
  MarkerSourceView *source_view = marker_editor_get_source_view(editor);
...
static void action_italic(GSimpleAction *action, GVariant *parameter,
                          gpointer window) {
  MarkerEditor *editor = marker_window_get_active_editor(MARKER_WINDOW(window));
  MarkerSourceView *source_view = marker_editor_get_source_view(editor);
  marker_source_view_surround_selection_with(source_view, "*");
...
static void action_bold(GSimpleAction *action, GVariant *parameter,
                        gpointer window) {
  MarkerEditor *editor = marker_window_get_active_editor(MARKER_WINDOW(window));
  MarkerSourceView *source_view = marker_editor_get_source_view(editor);
  marker_source_view_surround_selection_with(source_view, "**");
...
static void action_zoom_original(GSimpleAction *action, GVariant *parameter,
                                 gpointer user_data) {
  MarkerWindow *window = user_data;
  MarkerEditor *editor = marker_window_get_active_editor(window);
  MarkerPreview *preview = marker_editor_get_preview(editor);
...
static void action_editor_only_mode(GSimpleAction *action, GVariant *parameter,
                                    gpointer user_data) {
  MarkerWindow *window = user_data;
  MarkerEditor *editor = marker_window_get_active_editor(window);
...
static void action_preview_only_mode(GSimpleAction *action, GVariant *parameter,
                                     gpointer user_data) {
  MarkerWindow *window = user_data;
  MarkerEditor *editor = marker_window_get_active_editor(window);
...
static void action_dual_pane_mode(GSimpleAction *action, GVariant *parameter,
                                  gpointer user_data) {
  MarkerWindow *window = user_data;
  MarkerEditor *editor = marker_window_get_active_editor(window);
...
static void action_dual_window_mode(GSimpleAction *action, GVariant *parameter,
                                    gpointer user_data) {
  MarkerWindow *window = user_data;
  MarkerEditor *editor = marker_window_get_active_editor(window);
...
gchar *make_markup_title(MarkerEditor *editor, const char *raw_title) {
  gchar *markup_title;
...
static void title_changed_cb(MarkerEditor *editor, const gchar *title,
                             const gchar *raw_title, gpointer user_data) {
  ...
  if (get_current_iter(window, &iter)) {
    g_autofree gchar *markup_title = make_markup_title(editor, raw_title);
    gtk_tree_store_set(window->documents_tree_store, &iter, TITLE_COLUMN,
                       markup_title, -1);
  }
}

static void subtitle_changed_cb(MarkerEditor *editor, const gchar *subtitle,
                                gpointer user_data) {
  MarkerWindow *window = MARKER_WINDOW(user_data);
  gtk_header_bar_set_subtitle(window->header_bar, subtitle);
}

static void preview_zoom_changed_cb(MarkerPreview *preview,
                                    gpointer user_data) {
  MarkerWindow *window = user_data;
...
static gboolean window_deleted_event_cb(GtkWidget *widget, GdkEvent *event,
                                        gpointer user_data) {
  MarkerWindow *window = user_data;
...
static void rename_file_action_cb(GtkCellRendererText *cell, gchar *path_string,
  ...
  if (gtk_tree_model_get_iter_from_string(model, &iter, path_string)) {
    ...
    if (marker_editor_rename_file(editor, g_strdup(new_text))) {
      g_autofree gchar *markup_title = make_markup_title(editor, new_text);
      gtk_tree_store_set(window->documents_tree_store, &iter, TITLE_COLUMN,
                         markup_title, -1);
...
static void tree_selection_changed_cb(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, EDITOR_COLUMN, &editor, -1);
    gtk_stack_set_visible_child_full(window->editors_stack, name,
                                     GTK_STACK_TRANSITION_TYPE_CROSSFADE);

    window->active_editor = editor;
    g_autofree gchar *title =
        marker_editor_get_title(marker_window_get_active_editor(window));
    g_autofree gchar *subtitle =
        marker_editor_get_subtitle(marker_window_get_active_editor(window));
    gtk_header_bar_set_title(window->header_bar, title);
    gtk_header_bar_set_subtitle(window->header_bar, subtitle);
    preview_zoom_changed_cb(marker_editor_get_preview(editor), window);
    g_free(name);
  }
}

static gboolean close_button_clicked(GtkTreeView *view, GtkTreeViewColumn *col,
                                     guint x, GtkCellRenderer *cell) {
  ...
  gtk_cell_renderer_get_preferred_width(checkcell, GTK_WIDGET(view), &min_width,
                                        &nat_width);
...
static gboolean button_pressed_cb(GtkWidget *view, GdkEventButton *bevent,
                                  gpointer data) {

  GtkCellRenderer *cell_renderer = GTK_CELL_RENDERER(data);
  MarkerWindow *window =
      MARKER_WINDOW(gtk_widget_get_ancestor(view, MARKER_TYPE_WINDOW));
  gint x;

  GtkTreePath *path = gtk_tree_path_new();
  GtkTreeViewColumn *col = gtk_tree_view_column_new();

  gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(view), bevent->x, bevent->y,
                                &path, &col, &x, NULL);
...
static void marker_window_init(MarkerWindow *window) {
  { // SETUP ACTIONS //
    GAction *action = NULL;
    GtkApplication *app = marker_get_app();

    action = G_ACTION(g_simple_action_new("refresh", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_refresh), window);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_open_sketcher), window);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_search), window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_italic), window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_monospace), window);
    const gchar *monospace_accels[] = {"<Ctrl>m", NULL};
    gtk_application_set_accels_for_action(app, "win.monospace",
                                          monospace_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("new", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(marker_create_new_window), NULL);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_new_editor), window);
    const gchar *neweditor_accels[] = {"<Ctrl>n", NULL};
    gtk_application_set_accels_for_action(app, "win.neweditor",
                                          neweditor_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("closedocument", NULL));
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_close_current_document),
                             window);
    const gchar *closedocument_accels[] = {"<Ctrl>w", NULL};
    gtk_application_set_accels_for_action(app, "win.closedocument",
                                          closedocument_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("close", NULL));
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_try_close), window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_zoom_original), window);
    const gchar *zoomoriginal_accels[] = {"<Ctrl>equal", NULL};
    gtk_application_set_accels_for_action(app, "win.zoomoriginal",
                                          zoomoriginal_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("zoomin", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_zoom_in), window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_zoom_out), window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_editor_only_mode), window);
    const gchar *editoronlymode_accels[] = {"<Ctrl>1", NULL};
    gtk_application_set_accels_for_action(app, "win.editoronlymode",
                                          editoronlymode_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("previewonlymode", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_preview_only_mode), window);
    const gchar *previewonlymode_accels[] = {"<Ctrl>2", NULL};
    gtk_application_set_accels_for_action(app, "win.previewonlymode",
                                          previewonlymode_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("dualpanemode", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_dual_pane_mode), window);
    const gchar *dualpanemode_accels[] = {"<Ctrl>3", NULL};
    gtk_application_set_accels_for_action(app, "win.dualpanemode",
                                          dualpanemode_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("dualwindowmode", NULL));
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_dual_window_mode), window);
    const gchar *dualwindowmode_accels[] = {"<Ctrl>4", NULL};
    gtk_application_set_accels_for_action(app, "win.dualwindowmode",
                                          dualwindowmode_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new("open", NULL));
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_open_file), window);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_save_active_file),
                             window);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_window_save_active_file_as),
                             window);
    ...
    g_signal_connect_swapped(G_SIMPLE_ACTION(action), "activate",
                             G_CALLBACK(marker_exporter_show_export_dialog),
                             window);
    ...
    g_signal_connect(G_SIMPLE_ACTION(action), "activate",
                     G_CALLBACK(action_print), window);
    const gchar *print_accels[] = {"<Ctrl>p", NULL};
    gtk_application_set_accels_for_action(app, "win.print", print_accels);
    g_action_map_add_action(G_ACTION_MAP(window), action);

    action = G_ACTION(g_simple_action_new_stateful(
        "fullscreen", NULL, g_variant_new_boolean(FALSE)));
    g_signal_connect(G_SIMPLE_ACTION(action), "change-state",
                     G_CALLBACK(action_fullscreen), window);
    ...
    action = G_ACTION(g_simple_action_new_stateful(
        "sidebar", NULL, g_variant_new_boolean(FALSE)));
    g_signal_connect(G_SIMPLE_ACTION(action), "change-state",
                     G_CALLBACK(action_sidebar), window);
  ...
  gtk_builder_add_from_resource(
      builder, "/com/github/fabiocolacio/marker/ui/marker-window-main-view.ui",
      NULL);

  /** DOCUMENTS TREE **/
  GtkTreeView *documents_tree_view =
      GTK_TREE_VIEW(gtk_builder_get_object(builder, "documents_tree_view"));
  GtkTreeStore *documents_store =
      gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, GDK_TYPE_PIXBUF,
                         G_TYPE_STRING, MARKER_TYPE_EDITOR);
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "markup", TITLE_COLUMN,
                                      NULL);

  g_signal_connect(renderer, "edited", G_CALLBACK(rename_file_action_cb),
                   window);

  renderer = gtk_cell_renderer_pixbuf_new();
  gtk_tree_view_column_pack_start(column, renderer, FALSE);
  gtk_tree_view_column_set_attributes(column, renderer, "pixbuf", ICON_COLUMN,
                                      NULL);
  ...
  g_signal_connect(documents_tree_view, "button-press-event",
                   G_CALLBACK(button_pressed_cb), renderer);

  /** EDITOR STACKS **/
  window->editors_stack =
      GTK_STACK(gtk_builder_get_object(builder, "documents_stack"));
  gtk_widget_show(GTK_WIDGET(window->editors_stack));

  /** MAIN PANED **/
  GtkWidget *main_paned =
      GTK_WIDGET(gtk_builder_get_object(builder, "main_paned"));
  gtk_box_pack_start(vbox, main_paned, TRUE, TRUE, 0);
  ...
  gtk_builder_add_from_resource(
      builder, "/com/github/fabiocolacio/marker/ui/marker-headerbar.ui", NULL);
  GtkHeaderBar *header_bar =
  ...
  g_signal_connect_swapped(unfullscreen_btn, "clicked",
                           G_CALLBACK(marker_window_unfullscreen), window);
  ...
  gtk_builder_add_from_resource(
      builder, "/com/github/fabiocolacio/marker/ui/marker-gear-popover.ui",
      NULL);
  GtkWidget *popover =
      GTK_WIDGET(gtk_builder_get_object(builder, "gear_menu_popover"));
  window->zoom_original_btn =
      GTK_BUTTON(gtk_builder_get_object(builder, "zoom_original_btn"));
  ...
  if (!marker_has_app_menu()) {
    GtkWidget *extra_items_start = GTK_WIDGET(
        gtk_builder_get_object(builder, "appmenu_popover_items_start"));
    GtkWidget *extra_items_end = GTK_WIDGET(
        gtk_builder_get_object(builder, "appmenu_popover_items_end"));
    ...
    g_action_map_add_action_entries(G_ACTION_MAP(app), APP_MENU_ACTION_ENTRIES,
                                    APP_MENU_ACTION_ENTRIES_LEN, window);
  }
  ...
  g_signal_connect(window, "delete-event", G_CALLBACK(window_deleted_event_cb),
                   window);

  g_object_unref(builder);
}

static void marker_window_constructed(GObject *object) {
  G_OBJECT_CLASS(marker_window_parent_class)->constructed(object);
  gtk_application_window_set_show_menubar(GTK_APPLICATION_WINDOW(object),
                                          FALSE);
}

static void marker_window_class_init(MarkerWindowClass *class) {
  G_OBJECT_CLASS(class)->constructed = marker_window_constructed;
}

void marker_window_add_editor(MarkerWindow *window, MarkerEditor *editor) {
  window->active_editor = editor;
  ...
  gtk_tree_store_append(window->documents_tree_store, &iter,
                        NULL); /* Acquire an iterator */

  GdkPixbuf *icon =
      gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), "window-close", 16,
                               GTK_ICON_LOOKUP_FORCE_SYMBOLIC, NULL);

  gtk_tree_store_set(window->documents_tree_store, &iter, TITLE_COLUMN,
                     marker_editor_get_title(editor), ICON_COLUMN, icon,
                     NAME_COLUMN, name, EDITOR_COLUMN, editor, -1);

  gtk_tree_selection_select_iter(
      gtk_tree_view_get_selection(window->documents_tree_view), &iter);

  if (window->editors_counter >= 1) {
    marker_window_show_sidebar(window);
  }

  g_signal_connect(editor, "title-changed", G_CALLBACK(title_changed_cb),
                   window);
  g_signal_connect(editor, "subtitle-changed", G_CALLBACK(subtitle_changed_cb),
                   window);
...
void marker_window_new_editor(MarkerWindow *window) {
  ...
  if (window->untitled_files) {
    marker_editor_rename_file(
        editor, g_strdup_printf("Untitled_%u.md", window->untitled_files));
  }
  window->untitled_files++;
  marker_window_add_editor(window, editor);
}

void marker_window_new_editor_from_file(MarkerWindow *window, GFile *file) {
  GList *children =
      gtk_container_get_children(GTK_CONTAINER(window->editors_stack));
...
MarkerWindow *marker_window_new(GtkApplication *app) {
  MarkerWindow *window =
      g_object_new(MARKER_TYPE_WINDOW, "application", app, NULL);
  marker_window_new_editor(window);
  return window;
}

MarkerWindow *marker_window_new_from_file(GtkApplication *app, GFile *file) {
  MarkerWindow *window =
      g_object_new(MARKER_TYPE_WINDOW, "application", app, NULL);
...
void marker_window_open_file(MarkerWindow *window) {
  ...
  g_autoptr(GtkFileChooserNative) dialog = gtk_file_chooser_native_new(
      _("Open"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_OPEN, _("_Open"),
      _("_Cancel"));
...
gboolean marker_window_is_active_editor(MarkerWindow *window,
                                        MarkerEditor *editor) {
  g_assert(MARKER_IS_WINDOW(window));
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      if (!gtk_tree_selection_get_selected(selection, &model, &iter)) {
        gint rows = gtk_tree_model_iter_n_children(
            GTK_TREE_MODEL(window->documents_tree_store), NULL);
        if (rows) {
          gtk_tree_selection_select_path(
              selection, gtk_tree_path_new_from_indices(rows - 1, -1));
        }
      }
    } else {
      /** close if model is empty **/
      if (!gtk_tree_model_get_iter_first(
              GTK_TREE_MODEL(window->documents_tree_store), &iter)) {
        marker_window_try_close(window);
      }
    }
  }
...
void marker_window_hide_sidebar(MarkerWindow *window) {
  if (window->sidebar_visible) {
    ...
    gtk_container_remove(GTK_CONTAINER(window->main_paned),
                         GTK_WIDGET(window->paned1));
